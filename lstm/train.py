...
model = models.DMVSTNet(input_size=20*10*2, temporal_hidden_size=temporal_hidden_size)
model.to(device)
...
for epoch in range(n_epochs): 
    ...
    for phase in ['train', 'val']:   
        ...
        for batch_x, batch_y in dataloaders[phase]:
            ...
            optimizer.zero_grad()
            # spatial_hidden = torch.zeros(window_size, len(inputs), 32, grid_size[0], grid_size[1]).to(device).float() ###
            temporal_hidden_cell = (torch.zeros(1, len(inputs), temporal_hidden_size).to(device),
                                 torch.zeros(1, len(inputs), temporal_hidden_size).to(device))
            
            with torch.set_grad_enabled(phase == 'train'):
                outputs = model(inputs, temporal_hidden_cell)
