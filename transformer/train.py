...
import transformer as models # transformer.py
...
for epoch in range(n_epochs): 
    ...
    for phase in ['train', 'val']:   
        ...
        for batch_x, batch_y in dataloaders[phase]:
            ...
            trans_hidden_cell = (torch.zeros(1, len(inputs), temporal_hidden_size).to(device),
                                 torch.zeros(1, len(inputs), temporal_hidden_size).to(device))
            
            with torch.set_grad_enabled(phase == 'train'):
                outputs = model(inputs, spatial_hidden, trans_hidden_cell)
